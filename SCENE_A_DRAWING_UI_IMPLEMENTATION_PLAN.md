...
팔레트는 고정 색상 버튼 8개가 아니라, 초기 RGB값을 가진 수정 가능 슬롯 8개다.

원칙:

- 시작 시 8칸은 모두 기본 색으로 채워져 있어야 한다.
- 각 기본 색은 명확한 RGB값을 가진다.
- 플레이어가 팔레트 한 칸을 클릭하면 해당 슬롯의 현재 색이 선택된다.
- 선택된 색은 Pen/Brush로 그릴 때 바로 적용된다.
- 플레이어가 팔레트 한 칸을 선택한 뒤 `COLOR` 박스의 RGB 기능을 조작하면, 그 슬롯의 RGB값이 변경된다.
- 슬롯 색이 변경되면 팔레트 칸의 표시 색, 현재 선택 색, 그리기 색이 모두 같이 갱신된다.
- 즉 팔레트는 `초기값은 정해져 있지만, 런타임에 바뀔 수 있는 8개 Color 데이터`로 관리한다.

초기 기본색은 `ui 초안 최종.png`에 보이는 팔레트 색 배치를 따른다.
...
정확한 RGB값은 구현 시 `ui 초안 최종.png` 또는 팔레트 기준 이미지에서 샘플링해 상수로 기록한다.

예시 구조:

```csharp
Color[] paletteColors = new Color[8]
{
    new Color32(...), // slot 0
    new Color32(...), // slot 1
    new Color32(...), // slot 2
...
- 하지만 현재 구현 기준은 `ui 초안 최종.png`와 같은 배치/색이므로, 초기 팔레트도 기준 이미지의 8칸을 따른다.
- 사용자가 RGB로 슬롯을 바꾸면 그 이후부터는 해당 슬롯의 변경된 RGB값을 현재 색으로 사용한다.
...
editingPaletteIndex = 2
currentDrawColor = paletteColors[2]
```
...
```text
paletteColors[2] 변경
PaletteSlot_2 표시 색 변경
currentDrawColor 변경
DrawingCanvas.brushColor 변경
```
