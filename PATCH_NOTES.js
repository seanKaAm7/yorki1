 ...
 * 다음 날 루프 연결 - `다음 날` 버튼으로 Day 증가, 일일 집계와 시각 초기화, 에너지 회복, 고로부터 손님 큐 재시작
 * 정산 진행 잠금 - 정산 모달은 ESC나 배경 클릭으로 닫히지 않고 `다음 날` 버튼으로만 진행
 * 빌더 정합성 보강 - 사용자 수동 조정 Customer / DialogueBox 좌표를 TalkSceneBuilder 기본값에 반영
 * 검증 - Assembly-CSharp / Assembly-CSharp-Editor 빌드 경고 0 오류 0, Unity refresh/compile, Yorki/Build Talk Scene, TalkScene validate clean, git diff --check clean
---------
*/
/*
---------
