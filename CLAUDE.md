...
- 장르: 캐리커처 화가 감성 시뮬레이션
- 기획 문서: `GAME_PLAN.md`, `SCENE_A_PLAN.md`
...
## 6. 현재 구현 상태
- SceneA.unity — 손님 대화 씬 (진행 중)
  - 배경, 손님 캐릭터(8종 스프라이트), FadeIn, 9-slice 대화창
  - SceneADialogue: 대사 20줄, 타이핑 출력, 포즈 그룹 시스템, ▼ 깜빡임
  - CustomerDisplay: 감정별 idle/talk 스프라이트 교체, Shake 연출
- 드로잉 캔버스 (DrawingCanvas.cs) — 마우스 드로잉, Undo 20단계, 지우개, 색상 변경
- 채점 알고리즘 (ScoreCalculator.cs) — 8x8 코스 그리드 + 슬라이딩 윈도우 ±2
- 5단계 반응 시스템 (ReactionSystem.cs, ReactionUI.cs)
- GameManager: Narrator ↔ Drawing 상태 전환
- 씬 자동 빌드: Yorki/Build Scene A, Yorki/Build Drawing Scene, Yorki/Setup Game Scene
